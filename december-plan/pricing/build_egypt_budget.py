"""Egypt group budget, Red Sea Diving Safari (Marsa Shagra) basis, first cut 24 Sep 2026 on Riad Tarek's indicative
low-season prices (email 08:34 UTC, thread 1a0ca71a4aee743e). Six-night quote scaled to our five nights; several
inputs are ASSUMED until RSDS answers the 24 Sep questions (four-day package, rental rate, five-night rate, deposit).
...
Flags: QUOTED = RSDS email 24 Sep; ADVERTISED = public page; ASSUMED = our estimate pending RSDS's answer.
"""
...
inp('room_pp', "Royal Tent, double, full board, per person per night", 78, "EUR/night", "QUOTED (six-night quote; five-night rate asked)", MONEY)
inp('nights', "Nights, Sun 3 – Fri 8 Jan", 5, "nights", "programme")
inp('pkg', "House reef diving package, 5 days unlimited (certified)", 347, "EUR", "QUOTED; four-day package asked, keep 5-day until answered", MONEY)
inp('day_extra', "Extra house-reef day (used for the new divers' Thursday after certification)", 69, "EUR", "QUOTED as the 6th-day add-on; ASSUMED usable as a single day", MONEY)
...
inp('rental', "Full equipment rental per day, certified divers (course prices include gear)", 20, "EUR/day", "ASSUMED; asked 24 Sep", MONEY)
...
inp('dep_rsds', "RSDS deposit at booking", 0.25, "share", "ASSUMED; asked 24 Sep", PCT)
...
inp('assist', "Meet-and-assist at the airport (optional)", 45, "EUR", "QUOTE NEEDED; asked 24 Sep", MONEY)
...
rows = [
 ("Diving with RSDS (course + e-code + Thursday day, or 5-day house reef package)", f"={R['ow']}+{R['ecode']}+{R['day_extra']}", f"={R['pkg']}"),
 ...
 ("TRIP FEE, USD (rounded up to the nearest 100, as posted)", "=CEILING(B11,100)", "=CEILING(C11,100)"),
 ("", None, None),
...
sect("Deposits and cash (RSDS terms ASSUMED until answered)")
...
pay['A1'] = "Payment schedule (RSDS deposit and cancellation terms asked 24 Sep; dates provisional)"; pay['A1'].font = H
for row in [
 ("date", "what", "who"),
 ...
 ("by Fri 23 Oct", "RSDS deposit (share to confirm); e-codes issued to students", "club account"),
 ("Fri 13 Nov", "Participant balances due (fee less deposit, plus add-ons)", "participants"),
 ("early Dec", "RSDS final payment per its terms", "club account"),
 ...
 ("", "", ""),
 ("Refund ladder", "Mirror RSDS's cancellation terms once known; until then deposits refundable in full until the club pays RSDS, then only if the place is resold.", "policy"),
