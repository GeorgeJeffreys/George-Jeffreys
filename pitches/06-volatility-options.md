...
Sharpe 0.3–0.9 net; volatility 10–15%; return 5–12%. Worst plausible drawdown 20–25% in a season like 2026 where realised beats implied across the board. The wings make the loss survivable but mean *buying* the deepest out-of-the-money lottery tickets, which carry the richest premium: the real cost of the defined-risk constraint. Correlates with dispersion spikes and VIX jumps in earnings weeks, roughly the mirror of a long/short fundamental book, so a complement to Lazy Prices rather than a duplicate. At Sharpe 0.6 a live record needs about ten years to reach two standard errors; monitor instead the per-event implied-to-realised ratio, far less noisy.
